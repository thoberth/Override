...
## I
This program open the ```/home/users/level03/.pass``` and put the value in a buffer.

## II
The progam ask for ```login``` and ```password``` with fgets which is protected against buffer overflow so we cannot use this.\
There is a cmp between 0 and the result of ```strcspn(buf_file, buf_pass, 41)```\
if this cmp return 1 there is call to system with arg ```/bin/sh```, but we havent the value for passw

## III

At the end there is an unprotected printf -> ```printf(buf_user)``` here we can do a format string exploit to target the value in ```buf_file``` which is the content of ```/home/users/level03/.pass```

# Read from the stack

our stackframe is 288 -> ```<+4>:     sub    $0x120,%rsp```\
the buffer we want to print is at ```-0xa0(%rbp)``` -> 160\
so 288-160 = 128 = offset\
we know that the length of the buf_file is 41 - '\0' = 40\
(128/8) = 21+1 //// 40 / 8 = 5 -> 21+5 = 26 \
so we have to read up the 21th bytes, from the 22th bytes to the 26th

# Payload

we put ```%22$p %23$p %24$p %25$p %26$p``` at the username prompt
...
this is not finish yet we have to decode from hexa to Ascii and inverse the order:
